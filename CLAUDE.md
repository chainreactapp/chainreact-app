...
**Open audit task:** [v2-explicit-handler-testmode-audit-task.md](./learning/docs/v2-explicit-handler-testmode-audit-task.md). PR-V2C added a registry fallback to v2 with test-mode short-circuit, but v2's existing explicit-case handlers rely on per-handler Q8d (only ~12 handlers implement it). v2's `INTERCEPT_WRITES` is post-hoc decoration — does NOT block real provider calls. The audit enumerates every explicit dispatch path and adds parity tests for representative cases (Slack send_message, Sheets append, Discord send, Airtable create_record, Notion page/database actions, Gmail send, Calendar create event). Required before Phase 4 (parity tests) of the consolidation plan can claim completeness.
