...
| 5 | [lib/webhooks/google-processor.ts](../../lib/webhooks/google-processor.ts) (Calendar / Drive / Sheets / Docs) | Google webhooks | ✓ via #3 | ✓ local maps preserved + dispatcher (`skipDedup: true` for the 3 with rollback-on-error; `${driveFile.id}:${changeType}` for Docs which lacks local dedup) | ❌ | ✓ Migrated 2026-05-04 (PR-V2-WEBHOOK-GOOGLE) — 4 of 5 sites; test-session site at line 3042 uses different code path (engine.executeWorkflow), out of scope |
...
| 8 | [app/api/microsoft-graph/worker/route.ts](../../app/api/microsoft-graph/worker/route.ts) | MS Graph queue worker | ✓ via #3 | ✓ 3 layers preserved + dispatcher (`eventUniqueKey`) | ❌ | ✓ Migrated 2026-05-04 (PR-V2-WEBHOOK-MS-GRAPH) |
...
| 13 | [lib/integrations/discordGateway.ts](../../lib/integrations/discordGateway.ts) | Discord gateway WS event | ✓ via #3 | ✓ via #3 (`${guildId}:${memberId}:${joined_at}`) | ❌ | ✓ Migrated 2026-05-04 (PR-V2-WEBHOOK-DISCORD-GATEWAY) |
